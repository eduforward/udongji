...
(function () {
  const VERSION = 'v81 · 2026-09-14 17:45';
  // 배포 이력 — 새 배포마다 맨 앞에 한 줄 추가 (t = 푸시 시각, 한국시간)
  const HISTORY = [
    { v: 81, d: '2026-09-14', t: '17:45', c: ['KPI: 관리자는 나만/모든 상담사 비교, 일반 상담사는 내 성과 + 전체 상담사 평균만 (전체 건수·다른 이름 비노출)'] },
    ...
    { v: 78, d: '2026-09-13', t: '02:31', c: ['상담 1단계 인사말을 유입 경로별로 — Out은 "○○ 하시는 매장 맞으시죠?"(매장명 자동), Call-In·Chat-In은 매장 이름·업종을 직접 묻는 질문으로'] },
    ...
    { v: 73, d: '2026-09-13', t: '01:31', c: ['상담 재연락 저장: 내일 · 내일모레 · 이번 주 금 · 다음 주 월 · 1주/2주 뒤 · 다음 달 초 빠른 선택, 기타는 날짜 직접 입력 + 시간·상황 메모 — 날짜가 항상 정확히 저장돼 재연락 목록 정렬에 반영'] },
    ...
    { v: 68, d: '2026-09-13', t: '00:45', c: ['사업자등록증 업로드 시 자동 판독(OCR, 브라우저 안에서만) — 사업자번호·사업장 소재지·법인등록번호·상호·성명·개업일을 읽어 확인 후 반영, "등록증 읽기" 버튼으로 다시 실행', '반영하면 페이앤 접수 폼 사업자번호·법인번호에 바로 들어감'] },
    ...
    { v: 59, d: '2026-09-12', t: '23:03', c: ['페이앤 이관 탭 아래에 업로드한 서류 사진 모아보기 — "복사" 후 페이앤 파일 칸에 붙여넣기(⌘V), 안 되면 다운로드'] },
    { v: 58, d: '2026-09-12', t: '22:53', c: ['고객 이관: 고객 목록에서 개별(상세 → 이관) 또는 목록 전체 이관 요청 → 받는 사람이 홈에서 개별/전부 수락해야 상담자가 바뀜', '홈 하단에 받은 이관 요청(수락·거절)과 내가 보낸 대기 요청(취소) 표시, 고객 상세에 이관 이력'] },
    { v: 57, d: '2026-09-12', t: '22:44', c: ['고객 목록 진행 상태 재정의 — 포기 / 재연락 / 수취자료진행 / 페이앤진행 / 전자서명진행 / 커넥트진행 / 완료 (계약 단계에서 자동 판정)', '"상태"·"페이앤 등록" 두 열을 "진행 상태" 하나로 통합, 상단 필터도 재연락·진행 중·완료·포기로'] },
    ...
    { v: 48, d: '2026-09-12', c: ['상담 스크립트 0단계 "유입 경로" 추가 — Call-In / Chat-In / Out 필수 선택, 경로별 인사말 자동 변경', '고객 목록·페이앤 접수 양식에 유입 경로 표시, KPI 대시보드에 경로 필터'] },
