...
(function () {
  ...
  const css = `
    :host { display: block; position: sticky; top: 0; z-index: 50; font-family: 'Pretendard Variable', Pretendard, -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif; letter-spacing: -0.01em; }
  ...
  class UNav extends HTMLElement {
    connectedCallback() {
      ...
      root.innerHTML = `<style>${css}</style><div class="bar"><div class="in">
        ...
        <nav>${PAGES.map(p => `<a href="${href(p)}" class="${p.key === cur ? 'on' : ''}">${p.label}</a>`).join('')}</nav>
        <div class="right"><span class="who" id="who"></span><button type="button" id="out" hidden>로그아웃</button></div>
      </div></div>`;
      const who = root.getElementById('who'), out = root.getElementById('out');
      const lib = this.getAttribute('lib') || './udongji-fb.js?v=2';
      import(lib).then(async m => { await m.init(); if (m.isSignedIn()) { who.textContent = m.userEmail(); out.hidden = false; out.onclick = async () => { await m.signOut(); location.href = href(PAGES[0]); }; } }).catch(() => {});
    }
  }
